...
let autofillBtnNew = document.getElementById("autofill-new");
let autofillBtnClassic = document.getElementById("autofill-classic");
...
// function addLog(msg, targetDocument) {
//     var thisdocument = document
//     if (targetDocument) {
//         thisdocument = targetDocument
//     }

//     var tr = thisdocument.createElement("tr")
//     var rowTd = thisdocument.createElement("td")
//     rowTd.setAttribute("scope", "row")
//     rowTd.innerHTML = row
//     var rowTdMsg = thisdocument.createElement("td")
//     rowTdMsg.innerHTML = msg
//     var rowTdDate = thisdocument.createElement("td")
//     rowTdDate.innerHTML = dateFormat("YYYY-mm-dd HH:MM:SS", new Date())
//     tr.appendChild(rowTd)
//     tr.appendChild(rowTdMsg)
//     tr.appendChild(rowTdDate)
//     logTable.appendChild(tr)
//     row ++;
// }

async function autofill(inject_func) {
    addLog("reading sheet start")
    let resp = await fetch('https://gsheet.moridisa.com/autofirebase/reading_sheet');
    let resp_json = await resp.json();
    addLog("reading sheet done")
...
async function inject_func_new(noti_list) {
    row = 1
    function sleep(time) {
        var basic_time = 1000
        if (time != null) {
    ...
    for (let index = 0; index < noti_list.length; index++) {
        ...
        // create new noti
        document.getElementById("new-msg-btn").click()
        await sleep()

        document.getElementsByClassName("new-campaign-label")[0].click();
        await sleep()
        ...
        // 触发选择app
        // addLog("trgger app select", pluginDocument)
...
async function inject_func_classical(noti_list) {
    function sleep(time) {
        var basic_time = 1000
        if (time != null) {
            basic_time *= time
...
async function autofill_new() {
    return await autofill(inject_func_new)
}

async function autofill_classic() {
    return await autofill(inject_func_classical)
}

autofillBtnNew.addEventListener("click", autofill_new);
autofillBtnClassic.addEventListener("click", autofill_classic);
